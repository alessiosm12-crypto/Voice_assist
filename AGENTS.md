...
- Финальная проверка перед копированием должна оставаться локальной и не заменять слова; она может менять только пунктуацию, пробелы, регистр начала предложений и границы предложений.
- Пользовательский словарь хранится в браузере через `localStorage` под ключом `voiceAssist.dictionary.v1`; не добавляйте серверное хранение без отдельной причины и обновления документации.
- Словарь должен работать консервативно: заменять только достаточно похожие распознанные варианты и не превращаться в агрессивную автозамену произвольных слов.
...
- Перед копированием используется режим предпросмотра: сначала локальная проверка, затем ручная правка при необходимости, затем копирование.
- Пользовательский словарь является fuzzy-постобработкой распознанного текста, а не обучением Web Speech API.
...
- Для изменений интерфейса или клавиатуры вручную проверьте в Chrome или Edge:
  ...
  - "Готово" открывает предпросмотр, а не копирует сразу.
  - В предпросмотре текст можно редактировать, "Скопировать" копирует текст в буфер обмена и закрывает окно, "Вернуться" возвращает к редактированию/диктовке.
...
- Для изменений пользовательского словаря проверяйте:
  - добавление выделенного термина на паузе;
  - сохранение словаря после перезапуска окна;
  - консервативную замену похожих распознанных вариантов;
  - очистку словаря;
  - отсутствие неожиданных замен обычных слов.
...
- Пользователь хочет повышать качество распознавания через собственный словарь имён, компаний и профессиональных терминов.
...
- Ошибки Web Speech API `audio-capture`, `not-allowed`, `service-not-allowed`, `network` и `language-not-supported` должны останавливать автоперезапуск распознавания, иначе окно может хаотично переключаться в паузу и обратно.
...
- Web Speech API не принимает пользовательский словарь напрямую; для терминов проекта используется локальная fuzzy-постобработка перед пунктуацией и капитализацией.
- Словарь v1 хранится в `localStorage`, поэтому он привязан к конкретному браузеру и профилю пользователя.
- Финальная локальная проверка пунктуации выполняется перед копированием и показывается пользователю в предпросмотре, чтобы эвристические исправления можно было вручную принять или поправить.
